...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorsMultiplication = x1 * x2 + y1 * y2;
  const vectorsModuleX = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorsModuleY = Math.sqrt(x2 ** 2 + y2 ** 2);
  const corner = vectorsMultiplication / (vectorsModuleX * vectorsModuleY);
...
function getParallelepipedDiagonal(a, b, c) {
  const squaredDiagonal = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(squaredDiagonal);
}
...
function roundToPowerOfTen(num, pow) {
  const roundNumber = Math.round(num / 10 ** pow);
...
function toNumber(value, def) {
  ...
  return Number.isFinite(valueIsNumber) ? valueIsNumber : def;
}
...
function numberToStringInBase(number, base) {
  const numberToString = number.toString(base);
...
function toFixed(number, fractionDigits) {
  const numberToFixed = number.toFixed(fractionDigits);
...
function toPrecision(number, precision) {
  const numberToPrecision = number.toPrecision(precision);
...
function isInteger(number) {
  const quantity = Number.isInteger(number);
  return quantity;
}
